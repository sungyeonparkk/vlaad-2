...
class UniAD(UniADTrack):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if context_head:
            self.context_head = build_head(context_head)
    ...
    def forward_train(self,
        ...
        if self.with_context_head:
            ## TODO: CONTEXT QUERY ## 
            outs_context = self.context_head.inference(imgs)

        # Forward Plan Head
        if self.with_planning_head:
            outs_planning = self.planning_head.forward_train(bev_embed, outs_motion, outs_context, sdc_planning, sdc_planning_mask, command, gt_future_boxes)
