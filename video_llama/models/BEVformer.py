...
#from mmdet3d.models.detectors import builder

#from projects.mmdet3d_plugin.core.bbox.util import normalize_bbox
#from projects.mmdet3d_plugin.models.utils.grid_mask import GridMask
from utils.grid_mask import GridMask
from utils.util import normalize_bbox
from utils import normalize_bbox, GridMask
...
class BEVFormerHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.img_backbone = builder.build_backbone(
            dict(
                type="ResNet",
                depth=101,
                num_stages=4,
            ...
            if self.freeze_bev_encoder:
                with torch.no_grad():
                    bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                        mlvl_feats=img_feats, img_metas=img_metas_single, prev_bev=prev_bev)
            else:
                bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                        mlvl_feats=img_feats, img_metas=img_metas_single, prev_bev=prev_bev)
    ...
    def loss(self,
        ...
        all_gt_bboxes_ignore_list = [
            gAt_bboxes_ignore for _ in range(num_dec_layers)
        ]
...
def bev_former(pretrained=False):
    _dim_ = 256
    _pos_dim_ = _dim_ // 2
    _ffn_dim_ = _dim_ * 2
    _num_levels_ = 4
    ...
    if pretrained:
        if not os.path.exists("/input/ckpt/bevformer_r101_dcn_24ep.pth"):
            print(
                "Downloading bevformer weights to /input/ckpt/bevformer_r101_dcn_24ep.pth ..."
            )
            #os.makedirs(".checkpoints", exist_ok=True)
            torch.hub.download_url_to_file(
                "https://github.com/zhiqi-li/storage/releases/download/v1.0/bevformer_r101_dcn_24ep.pth",
                "/input/ckpt/bevformer_r101_dcn_24ep.pth",
                progress=True,
            )

        model.load_state_dict(torch.load("/input/ckpt/bevformer_r101_dcn_24ep.pth"))
