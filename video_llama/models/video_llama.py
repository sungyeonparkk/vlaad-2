...
class VideoLLAMA(Blip2Base):
    ...
    def __init__(
        ...
        self.llama_tokenizer.add_tokens(
            [DEFAULT_AUDIO_PATCH_TOKEN], special_tokens=True
        )

        self.IMAGE_PATCH_TOKEN_ID = self.llama_tokenizer.get_vocab()[
            DEFAULT_IMAGE_PATCH_TOKEN
        ]
        self.AUDIO_PATCH_TOKEN_ID = self.llama_tokenizer.get_vocab()[
    ...
    def encode_videoQformer_audiovideo(self, image, audio):
        ...
        with self.maybe_autocast():
            ...
            (
                audio_feature,
                audio_imagebind_finalout,
            ) = self.audio_encoder.get_audio_feature(
                audio, modality_type=ModalityType.AUDIO
